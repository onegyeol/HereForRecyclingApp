...
export default function CDGuide(): React.JSX.Element {
  const [fontSize, setFontSize] = useState(14);
  const [isSpeaking, setIsSpeaking] = useState(false);
  const shouldStopRef = useRef(false);
  const descriptions = [
    'CD 등의 디스크는',
    '재활용이 어렵기에 일반쓰레기로 배출해요.',
  ];
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tabContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            ...
            <TouchableOpacity style={styles.tab} onPress={() => router.push('/waste/cd' as const)}>
              <Text style={[styles.tabText_selected, { fontSize }]}>CD</Text>
            </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <Image source={require('../../assets/images/guideline/cd1.jpg')} style={styles.image} />
        <Text style={[styles.description, { fontSize }]}>
          CD 등의 디스크는
        </Text>

        <Image source={require('../../assets/images/guideline/cd2.jpg')} style={styles.image} />
