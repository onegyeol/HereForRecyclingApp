...
export default function SeedGuide(): React.JSX.Element {
  const [fontSize, setFontSize] = useState(14);
  const [isSpeaking, setIsSpeaking] = useState(false);
  const shouldStopRef = useRef(false);
  const descriptions = [
    '딱딱한 껍질이나 건조된 과일 껍질은',
    '음식물이 아니기에 일반 쓰레기로 배출해요',
  ];
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tabContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            ...
            <TouchableOpacity style={styles.tab} onPress={() => router.push('/waste/seed' as const)}>
              <Text style={[styles.tabText_selected, { fontSize }]}>단단한 껍질류</Text>
            </TouchableOpacity>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <Image source={require('../../assets/images/guideline/seed1.jpg')} style={styles.image} />
        <Text style={[styles.description, { fontSize }]}>
          딱딱한 껍질이나{"\n"}
          건조된 과일 껍질은
        </Text>

        <Image source={require('../../assets/images/guideline/seed2.jpg')} style={styles.image} />
