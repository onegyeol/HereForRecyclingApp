...
export default function RmaenVinylGuide(): React.JSX.Element {
  const [fontSize, setFontSize] = useState(16);
  const [isSpeaking, setIsSpeaking] = useState(false);
  const shouldStopRef = useRef(false);
  const descriptions = [
    '라면 봉지는',
    '안의 쓰레기들을 버리고 깨끗하게 씻어줘요',
    '흩날리지 않도록 비닐들을 한꺼번에 모아서 비닐로 배출해요.',
  ];
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
          ...
          >
            <Text style={[styles.tabText_selected, { fontSize }]}>라면봉지</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <ScrollView
        ...
        <Image source={require('../../assets/images/guideline/ramen1.jpg')} style={styles.image} />
        <Text style={[styles.description, { fontSize }]}>
          라면 봉지는{"\n"}
        </Text>

        <Image source={require('../../assets/images/guideline/ramen2.jpg')} style={styles.image} />
        <Text style={[styles.description, { fontSize }]}>
          안의 쓰레기들을 버리고{"\n"}
          깨끗하게 씻어줘요.
        </Text>

        <Image source={require('../../assets/images/guideline/ramen3.jpg')} style={styles.image} />
